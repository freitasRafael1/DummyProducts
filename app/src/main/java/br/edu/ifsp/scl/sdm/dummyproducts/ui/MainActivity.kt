...
class MainActivity : AppCompatActivity() {
        ...
        amb.productsSp.apply {
            ...
            onItemSelectedListener = object: AdapterView.OnItemSelectedListener {
                ...
                override fun onItemSelected(
                    ...
                    retriveProductsImages(productList[position])
                }
    ...
    private fun retriveProducts() =    Thread {
            val productsConnection = URL(PRODUCTS_ENDPOINT).openConnection() as HttpsURLConnection
            try {
                if (productsConnection.responseCode == HTTP_OK) {
                    ...
                    runOnUiThread {  Toast.makeText(this, getString(R.string.request_problem), Toast.LENGTH_SHORT)
                        .show()}
                }
            } catch (ioe: IOException) {
                runOnUiThread {  Toast.makeText(this, getString(R.string.connection_failed), Toast.LENGTH_SHORT)
                    .show()}
            } catch (jse: JsonSyntaxException) {
                runOnUiThread {  Toast.makeText(this, getString(R.string.response_problem), Toast.LENGTH_SHORT)
                    .show()}
            } finally {
    ...
    private fun retriveProductsImages(product: Product) = Thread{
        product.images.forEach { imageUrl ->
            val imageConnection = URL(imageUrl).openConnection() as HttpsURLConnection
            ...
            try {
                if (imageConnection.responseCode == HTTP_OK) {
                    BufferedInputStream(imageConnection.inputStream).let{
                        ...
                        runOnUiThread {
                            productImagesList.add(imageBitmap)
